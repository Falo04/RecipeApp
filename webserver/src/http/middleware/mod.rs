...
///     - Add a function `async fn simple_call(self, req: Request) -> ControlFlow<Response, Request>`
///
///       It will be called for every request and may decide to return early with a response or
///       continue with the wrapped handlers by passing the request.
/// 2. Invoke `impl_simple_axum_layer!(MyMiddlewareLayer => MyMiddlewareService);`
...
///     world: &'static str,
/// }
/// impl_simple_axum_layer!(MyMiddlewareLayer => MyMiddlewareService);
...
/// ```
#[macro_export]
macro_rules! impl_simple_axum_layer {
    (
        $Layer:ident => $Service:ident
    ) => {
        $crate::impl_axum_layer!($Layer => $Service);
        impl $Layer {
            async fn call<S>(self, mut inner: S, request: ::axum::extract::Request) -> Result<::axum::response::Response, ::std::convert::Infallible>
            where
            ...
            {
                match self.simple_call(request).await {
                    ::std::ops::ControlFlow::Break(response) => Ok(response),
                    ::std::ops::ControlFlow::Continue(request) => inner
                        .call(request)
                        .await
...
/// Slightly more general and less convenient version of [`impl_simple_axum_layer!`](crate::impl_simple_axum_layer).
///
/// # Usage
/// Just like [`impl_simple_axum_layer!`](crate::impl_simple_axum_layer#usage) but instead of `simple_call`
...
///     {
///         info!("Hello {} from my middleware", self.world);
///         inner.call(request).await.map_ok(IntoResponse::into_response)
///     }
/// }
/// Router::new().layer(MyMiddlewareLayer { world: "World" });
/// ```
...
macro_rules! impl_axum_layer {
    (
            ...
            S::Future: Send + 'static,
        {
            ...
            type Future = ::futures_lite::future::Boxed<
            ...
            fn call(&mut self, request: Request) -> Self::Future {
                ...
                ::std::boxed::Box::pin($Layer::call(layer, ready_inner, request))
            }
        }
    };
}
